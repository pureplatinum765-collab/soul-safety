...
// POST /api/setup with the bearer token to initialize the database
export default async function handler(req, res) {
  ...
  if (!configured.length || !configured.includes(token)) {
  ...
  try {
    ...
    for (const sql of tables) {
      ...
      results.push(error
        ? { sql: sql.slice(0, 60) + '...', error: error.message }
        : { sql: sql.slice(0, 60) + '...', status: 'ok' }
      );
    }

    const { error: seedError } = await db.from('users').upsert([
